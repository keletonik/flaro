...
function FocusCard({ points, loading }: { points: string[]; loading: boolean }) {
  if (loading) return (
    <div className="bg-card border border-border rounded-2xl p-5">
      <div className="flex items-center gap-2 mb-3"><span className="font-mono text-[11px] text-amber-500/60">⚡</span><span className="font-mono text-[10px] font-medium text-foreground uppercase tracking-widest">Today's Focus</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {pipelineGaps && pipelineGaps.totalAtRisk > 0 && (
            <div className="pipeline-gap bg-card border border-border rounded-2xl p-5">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="font-mono text-[11px] text-red-500/60">!!</span>
                  <span className="font-mono text-[10px] font-medium text-foreground uppercase tracking-widest">Revenue Leakage</span>
                </div>
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[10px] font-medium text-foreground uppercase tracking-widest">Tasks</span>
                <span className="font-mono text-[10px] text-muted-foreground tabular-nums">{todos.length}</span>
              </div>
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button onClick={addTodo} disabled={!newTodo.trim()} className={cn("w-8 h-8 rounded-md flex items-center justify-center transition-all shrink-0 font-mono text-sm", newTodo.trim() ? "bg-primary text-white hover:opacity-90" : "bg-muted text-muted-foreground/30")}>
                +
              </button>
            </div>
            <div className="space-y-1">
              {todos.map(t => (
                <div key={t.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-md hover:bg-muted/40 transition-colors group">
                  <button onClick={() => toggleTodo(t.id, t.completed)} className="shrink-0 font-mono text-[12px] text-muted-foreground/30 hover:text-primary transition-colors">
                    ○
                  </button>
                  <span className="text-[13px] text-foreground flex-1 truncate">{t.text}</span>
                  <span className={cn("font-mono text-[9px] font-medium px-1.5 py-0.5 rounded", t.priority === "Critical" ? "text-red-600 bg-red-50 dark:text-red-400 dark:bg-red-900/20" : t.priority === "High" ? "text-orange-600 bg-orange-50 dark:text-orange-400 dark:bg-orange-900/20" : "text-muted-foreground bg-muted")}>{t.priority}</span>
                  <button onClick={() => deleteTodo(t.id)} className="opacity-0 group-hover:opacity-100 p-0.5 font-mono text-[10px] text-muted-foreground hover:text-red-500 transition-all">×</button>
                </div>
              ))}
          ...
          <div className="bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <span className="font-mono text-[11px] text-primary/60">**</span>
                <span className="font-mono text-[10px] font-medium text-foreground uppercase tracking-widest">Notes</span>
                <span className="font-mono text-[10px] text-muted-foreground tabular-nums">{notes.length}</span>
              </div>
            ...
            <div className="flex gap-2 mb-3">
              ...
              <button onClick={addNote} disabled={!newNote.trim()} className={cn("w-8 h-8 rounded-md flex items-center justify-center transition-all shrink-0 font-mono text-sm", newNote.trim() ? "bg-primary text-white hover:opacity-90" : "bg-muted text-muted-foreground/30")}>
                +
              </button>
            </div>
            <div className="space-y-1">
              {notes.map(n => (
                <div key={n.id} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-muted/40 transition-colors group">
                  ...
                  <button onClick={() => markNoteDone(n.id)} className="opacity-0 group-hover:opacity-100 p-0.5 font-mono text-[10px] text-muted-foreground hover:text-emerald-500 transition-all" title="Mark done">
                    ✓
                  </button>
                  <button onClick={() => deleteNote(n.id)} className="opacity-0 group-hover:opacity-100 p-0.5 font-mono text-[10px] text-muted-foreground hover:text-red-500 transition-all" title="Delete">
                    ×
                  </button>
                </div>
              ))}
