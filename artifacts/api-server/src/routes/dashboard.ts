...
router.get("/dashboard/summary", async (req, res) => {
  ...
  const totalJobs = allJobs.filter(j => j.status !== "Done").length;
  const openNotes = allNotes.filter(n => n.status === "Open").length;

  res.json({ critical, high, open, doneToday, totalJobs, openNotes });
});
...
router.get("/dashboard/focus", async (req, res) => {
  ...
  if (!openJobs.length && !openNotesList.length) {
    res.json({
      points: ["All clear — no open jobs or notes. Good work, Casper."],
  ...
  try {
    const response = await anthropic.messages.create({
      ...
      messages: [{
        role: "user",
        content: `You are AIDE, Casper's personal operations assistant. Based on the following open jobs and notes, generate 3-5 concise bullet points for today's focus. Be direct and prioritise urgently. Australian English.
...
Return ONLY a JSON array of strings, no explanation, no markdown. Example: ["Do X urgently", "Follow up on Y", "Check Z"]`
    ...
    res.json({
      points: ["Unable to generate focus points at this time. Check your open jobs and notes manually."],
      generatedAt: new Date().toISOString(),
    });
  }
});
