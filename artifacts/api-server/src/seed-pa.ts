...
import { PROJECTS_V2_DDL_STATEMENTS } from "./seed-projects-v2-ddl";
...
export async function seedPaSurface(): Promise<void> {
  ...
  try {
    ...
    for (const stmt of PROJECTS_V2_DDL_STATEMENTS) {
      await client.query(stmt);
    }
    logger.info("PA + attachments + cycle times + projects v2 schema ensured");
  } catch (err) {
